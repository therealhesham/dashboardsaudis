...
const fetcher = await fetch('../api/unhide',{method:"post",headers: {'Accept':'application/json',
        "Content-Type": "application/json",
      },body:JSON.stringify({id:id})})

      const e= await fetcher.text()
if(fetcher.status == 200) {return setDeletedid(id)
...
// console.log(decoder.idnumber)
    ...
    try {
     ...
     await fetch("../api/hiddenlist").then(response => response.json())
  .then(json  => {
...
<TableCell>الغاء الاخفاء</TableCell>
          ...
          <TableBody>
            {paginatedData?.map((e, i) => (
                ...
                <Button onClick={()=>{Unhide(e.id)  }} style={{backgroundColor:"teal"}}>الغاء</Button>
                </TableCell>
